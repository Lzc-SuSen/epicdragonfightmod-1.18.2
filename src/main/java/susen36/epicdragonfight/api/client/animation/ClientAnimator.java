...
public class ClientAnimator extends Animator {
	...
	public void tick() {
		...
		if (!this.compareCompositeMotion(this.entitypatch.currentCompositeMotion)) {
			if (this.compositeLivingAnimations.containsKey(this.entitypatch.currentCompositeMotion)) {
				...
				this.getCompositeLayer(Priority.MIDDLE).off(this.entitypatch);
			}
		}
	...
	public Layer getCompositeLayer(Priority priority) {
	...
	public Pose getComposedLayerPose(float partialTicks) {
		...
		Map<Priority, Pair<DynamicAnimation, Pose>> layerPoses = Maps.newLinkedHashMap();
		layerPoses.put(Priority.LOWEST, Pair.of(this.baseLayer.animationPlayer.getAnimation(), currentBasePose));
		...
		for (Priority priority : this.baseLayer.baserLayerPriority.uppers()) {
	...
	public Pose getComposedLayerPoseBelow(Priority priorityLimit, float partialTicks) {
		Pose composedPose = this.baseLayer.animationPlayer.getCurrentPose(this.entitypatch, partialTicks);
		Map<Priority, Pair<DynamicAnimation, Pose>> layerPoses = Maps.newLinkedHashMap();
		
		for (Priority priority : priorityLimit.lowers()) {
			Layer compositeLayer = this.baseLayer.compositeLayers.get(priority);
	...
	public void applyBindModifier(Pose result, Joint joint, Map<Priority, Pair<DynamicAnimation, Pose>> poses) {
